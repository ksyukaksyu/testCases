"""
Cложность алгоритма: O(n) - линейная функция от каждого элемента списка,
потребление памяти: O(1) - вставка слева в связанный список, новых переменных внутри цикла не создается
Для массива: сложность O(n), память O(1) - ничего не изменилось, так как нам все еще нужно перебрать
